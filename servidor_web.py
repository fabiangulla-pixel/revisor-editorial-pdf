...
interfaz de web/. Todo corre en 127.0.0.1 — el PDF nunca sale de la
máquina salvo las llamadas a la API del proveedor de IA que el usuario
elija (igual que la app de escritorio).

Uso: python servidor_web.py  → abre el navegador en http://127.0.0.1:8420
"""
...
def _generar_entregables(ruta_pdf: str):
    estado = ESTADO
...
def reaplicar_filtro_documental() -> dict:
    estado = ESTADO
    ...
    if estado.dir_salida:
        _generar_entregables(estado.ruta_pdf_analizada)
...
class ManejadorAPI(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
            ...
            return self._json(
                {
                    ...
                    "total_hallazgos": len(ESTADO.hallazgos),
                    "log": ESTADO.log[-200:],
                    "dictamen": ESTADO.dictamen,
                    "entregables_listos": bool(ESTADO.entregables),
                    "costo_real": ESTADO.costo_real,
                    "perfil": {
                        "cargado": ESTADO.perfil.esta_cargado(),
                        "resumen": ESTADO.perfil.resumen_corto()
                        if ESTADO.perfil.esta_cargado()
                        else "",
                    },
                }
            )
            ...
            return self._json(
                {
                    ...
                    "modelo_actual": ESTADO.modelos,
                    "keys_configuradas": {k: bool(v) for k, v in ESTADO.api_keys.items()},
                    "autor": ESTADO.autor,
                }
            )
        ...
        if ruta.startswith("/api/entregables/descargar"):
            tipo = qs.get("tipo", [""])[0]
            ruta_archivo = ESTADO.entregables.get(tipo)
    ...
    def do_POST(self):
            ...
            try:
                ...
                for i in range(total_pag):
                    try:
                        ...
                        n_texto += 1
                modelo = ESTADO.modelos.get(proveedor_id)
        ...
        if ruta == "/api/iniciar_revision":
            ...
            ESTADO.entregables = {}
            ESTADO.dictamen = ""
            ESTADO.costo_real = None
            hilo = threading.Thread(
                target=ejecutar_revision, args=(ruta_pdf, proveedor_id), daemon=True
            )
            hilo.start()
            return self._json({"ok": True})

        if ruta == "/api/detener":
            ESTADO.detener_flag = True
            return self._json({"ok": True})
        ...
        if ruta == "/api/config":
            ...
            for prov in PROVEEDORES:
                if f"key_{prov}" in body:
                    ESTADO.api_keys[prov] = body[f"key_{prov}"]
                if f"modelo_{prov}" in body:
                    ESTADO.modelos[prov] = body[f"modelo_{prov}"]
            # Persistir en el mismo .env que usa la app de escritorio.
            try:
                lineas = []
                mapa = {
                ...
                for prov, env_key in mapa.items():
                    if ESTADO.api_keys.get(prov):
                        lineas.append(f"{env_key}={ESTADO.api_keys[prov]}")
...
def main():
    puerto = 8420
    servidor = ThreadingHTTPServer(("127.0.0.1", puerto), ManejadorAPI)
